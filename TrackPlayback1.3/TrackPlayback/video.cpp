...
const char *tString = GET_STR(
    ...
    void video::Repaint(AVFrame *frame)
    {
        if(!frame)return;
        mux.lock();
        if(!datas[0] || height*width==0 || frame->width!=this->width || frame->height!=this->height)
        {
            av_frame_free(&frame);
            mux.unlock();
            return;
    ...
    void video::Init(int width,int height)
    {
        mux.lock();
        this->width = width;
        this->height = height;
        delete datas[0];  //当多次打开时，释放上次的空间
        delete datas[1];
        delete datas[2];
        ///分配材质内存空间
...
mux.lock();
    ...
    // 窗口尺寸变化
    void video::resizeGL(int width, int height)
    {
        mux.lock();
